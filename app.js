...
function selectTier(tier) {
    ...
    const selected = document.querySelector(`.price-option[data-tier='${tier}']`);
...
function actualizarCotizacion() {
    ...
    const tierInst = currentTier === 'min' ? instMin : (currentTier === 'premium' ? instPremium : instIdeal);
    const tierMen = currentTier === 'min' ? menMin : (currentTier === 'premium' ? menPremium : menIdeal);

    animateValue(instMinSpan, instMin, '$');
    animateValue(instIdealSpan, tierInst, '');
    ...
    menIdealSpan.textContent = '$' + formatMoney(tierMen) + '/mes';
    ...
    comisionInstSpan.textContent = '$' + formatMoney(comisionInst);
    comisionMenSpan.textContent = '$' + formatMoney(comisionMen);
    ...
    const colors = { basico: '#10b981', operativo: '#3b82f6', pro: '#f59e0b' };
    complexityBadge.style.borderColor = colors[complejidadKey];
    complexityBadge.style.color = colors[complejidadKey];
    complexityBadge.style.background = colors[complejidadKey] + '20';

    const percentage = ((tierMen - menMin) / Math.max(menPremium - menMin, 1)) * 100;
...
function animateValue(element, value, prefix) {
    const start = parseInt(String(element.textContent).replace(/[^0-9]/g, '')) || 0;
    const end = value;
    const duration = 500;
...
function updateModules(giro, complejidadKey) {
    ...
    if (checkboxes.portal.checked && !modulosAMostrar.some(m => m.includes('Portal'))) modulosAMostrar.push('Portal del cliente');
    if (checkboxes.pdf.checked && !modulosAMostrar.some(m => m.includes('PDF'))) modulosAMostrar.push('Generación PDF automática');
    if (checkboxes.inventario.checked && !modulosAMostrar.some(m => m.includes('Inventario'))) modulosAMostrar.push('Sistema de inventario');
    if (checkboxes.dashboard.checked && !modulosAMostrar.some(m => m.includes('Dashboard'))) modulosAMostrar.push('Dashboard ejecutivo');
    if (checkboxes.fotos.checked && !modulosAMostrar.some(m => m.includes('Evidencias'))) modulosAMostrar.push('Evidencias fotográficas');
    if (checkboxes.whatsapp.checked && !modulosAMostrar.some(m => m.includes('WhatsApp'))) modulosAMostrar.push('Integración WhatsApp');
    ...
    modulosAMostrar.slice(0, 8).forEach((mod, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="module-text">${mod}</div>
            ${index > 5 ? '<span class="module-badge">Extra</span>' : ''}
        `;
...
Object.values(checkboxes).forEach(cb => cb.addEventListener('change', actualizarCotizacion));
...
document.getElementById('copyResumenBtn').addEventListener('click', function() {
    ...
    const resumen = `🚀 *Propuesta Comercial SDMX*\n\n` +
        `*Negocio:* ${giro}\n` +
        `*Zona:* ${zona}\n` +
        `*Tamaño:* ${tam}\n` +
        `*Nivel:* ${comp}\n\n` +
        ...
        `• Instalación: $${inst}\n` +
        `• Mensualidad: ${men}\n\n` +
        `📦 *Incluye:* ${modulosContainer.children.length} módulos especializados\n` +
